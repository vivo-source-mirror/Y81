...
extern unsigned long rsc_arch_get_cpu_efficiency(int cpu);
...
{
	return ((rsc_arch_get_cpu_efficiency(cpu) << 10) / rsc_min_possible_efficiency);
}
...
again:
	...
	if (!(cnt & WRITE_BIT_MASK)) {
		...
		if (debug_mode == CPUCAP_NORMAL_MODE) {
			...
			} else if (mode == RSC_CPU_UP || mode == RSC_CPU_DOWN) {
		...
		unsigned long val, void *v)
{
	...
	} else if (RSC_CPU_DOWN == mode || RSC_CPU_UP == mode) {
#ifdef RSC_CPUCAP_NOLOCK
		pos = rsc_cpucap_enqueue(rsc_cap);
		pd = &rsc_cap->data[pos];
#endif
...
__ATTR(CPUCAP_NOTIFY_NAME, 0640, show_rsc_cpucap_notif, store_rsc_cpucap_notif);

static struct kobj_attribute rsc_cpucap_notif_detail_attr =
__ATTR(CPUCAP_NOTIFY_DETAIL_NAME, 0640, show_rsc_cpucap_detail_notif, store_rsc_cpucap_detail_notif);

static struct kobj_attribute rsc_cpucap_notif_debug_attr =
__ATTR(CPUCAP_NOTIFY_DEBUG_NAME, 0640, show_rsc_cpucap_debug_notif, store_rsc_cpucap_debug_notif);
...
static int init_cpu_cluster(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		rsc_min_possible_efficiency = min(rsc_min_possible_efficiency, rsc_arch_get_cpu_efficiency(cpu));
		actcpu = cpu;
retry:

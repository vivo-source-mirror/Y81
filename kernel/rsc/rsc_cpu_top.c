...
again:
	...
	snprintf(m->buf, m->size, TOP_HEAD_FORMAT,
		...
		(u64)utime, (u64)stime, m->count, curjif,
		(time1 - time0)/1000, (time2 - time1)/1000);
	if (headcnt < m->size)
		m->buf[headcnt] = tmpc;
	mutex_unlock(&pid_lock);
	rsc_dbg(CPU_TOP, "%s: T: %d m->size: %lu count: %lu sizeof(rsc_cpu_top): %lu pcount: %d jiffs: %llu time:%llu %llu\n",
		__func__, type, m->size, m->count,  sizeof(inf), pcount, get_jiffies_64(),
		(time1 - time0)/1000, (time2 - time1)/1000);
...
static int stat_top_open_all(struct inode *inode, struct file *file)
{
	...
	size_t size = 64 * 1024;
	return single_open_size(file, stat_cpu_show_all, PDE_DATA(inode), size);
}
...
static int stat_top_open(struct inode *inode, struct file *file)
{
	...
	size_t size = 32 * 1024;
	return single_open_size(file, stat_cpu_show, PDE_DATA(inode), size);
}
...
#endif
{
	...
	"%s: rsc_total_pid: %d Comm: %16s(%d)"
		...
		tpid_entry->comm, tpid, utime, tpid_entry->utime,
		stime, tpid_entry->stime);

...
static int mmc_ocp_show(struct seq_file *s, void *data)
{
	struct mmc_host	*host = s->private;
	struct mmc_ios	*ios = &host->ios;
	const char *str;
	...
	if (ios->vtf_ocp)
		...
		str = "inactive";
	seq_printf(s, "vtf_ocp:\t%d (%s)\n", ios->vtf_ocp?1:0, str);
	seq_printf(s, "ocp_gpio:\t%d\n", ios->ocp_gpio);

	return 0;
}

static int mmc_ocp_open(struct inode *inode, struct file *file)
{
	return single_open(file, mmc_ocp_show, inode->i_private);
}

static const struct file_operations mmc_ocp_fops = {
	.open		= mmc_ocp_open,
	.read       = seq_read,
	.llseek     = seq_lseek,
	.release    = single_release,
...
void mmc_add_host_debugfs(struct mmc_host *host)
{
	...
	if (IS_ERR(fault_create_debugfs_attr("fail_mmc_request",
		...
		goto err_node;
#endif
	if (!debugfs_create_file("ocp", S_IRWXUGO, root, host,
		&mmc_ocp_fops))
		goto err_node;

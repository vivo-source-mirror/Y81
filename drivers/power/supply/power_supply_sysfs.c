...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH/* @vivo add for Dual-engine */
		"Safety timer expire", "Dual-engine"
#else
		"Safety timer expire"
#endif
	};
...
static struct device_attribute power_supply_attrs[] = {
	...
	POWER_SUPPLY_ATTR(instat_voltage),
	POWER_SUPPLY_ATTR(current),
	POWER_SUPPLY_ATTR(charger_voltage),
	POWER_SUPPLY_ATTR(charger_type),
	POWER_SUPPLY_ATTR(warning),
	POWER_SUPPLY_ATTR(fast_charge_enable),
...
int power_supply_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	for (j = 0; j < psy->desc->num_properties; j++) {
        ...
        if(psy->desc->properties[j] < (ARRAY_SIZE(power_supply_attrs)-1) )
			attr = &power_supply_attrs[psy->desc->properties[j]];
        else {
             dev_err(dev, "power_supply_uevent error %s-%d\n", psy->desc->name, j);
             goto out;
        }

...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		} else if (GF_KEY_HOME_IMAGE == gf_key.key) {
			key_input = GF_INPUT_SCREENSHOT_KEY;
		...
		if ((GF_KEY_POWER == gf_key.key || GF_KEY_CAMERA == gf_key.key) && (gf_key.value == 1)) {
			...
			input_sync(gf_dev->input);
		} else if ((GF_KEY_HOME_IMAGE == gf_key.key) && (gf_key.value == 1)) {
			input_report_key(gf_dev->input, GF_INPUT_SCREENSHOT_KEY, 1);
			input_sync(gf_dev->input);
		} else if ((GF_KEY_HOME_IMAGE == gf_key.key) && (gf_key.value == 0)) {
			input_report_key(gf_dev->input, GF_INPUT_SCREENSHOT_KEY, 0);
			input_sync(gf_dev->input);
		}
...
static int gf_probe(struct spi_device *spi)
{
    ...
    regulator_set_voltage(gf_dev->reg, 2800000, 2800000);
	gf_hw_power_enable(gf_dev, 1);
...
#endif
	if (get_fp_id() != GOODIX_GF3208 && get_fp_id() != GOODIX_GF318M) {
		printk("%s(): wrong gf3208 gf318m id, exit\n", __func__);
		return 0;
	}

...
extern int32_t nvt_extra_proc_init_1732(void);
#endif

#if NVT_TOUCH_MP
extern int32_t nvt_mp_proc_init_1732(void);
#endif

struct nvt_ts_data_1732 *ts_1732;

#if BOOT_UPDATE_FIRMWARE
static struct workqueue_struct *nvt_fwu_wq;
extern void Boot_Update_Firmware_1732_1732(struct work_struct *work);
#endif
...
#endif
#endif
static const struct nvt_ts_mem_map_1732 NT36672A_memory_map = {
...
static const struct nvt_ts_mem_map_1732 NT36772_memory_map = {
...
static const struct nvt_ts_mem_map_1732 NT36525_memory_map = {
...
static const struct nvt_ts_mem_map_1732 NT36870_memory_map = {
...
static const struct nvt_ts_mem_map_1732 NT36676F_memory_map = {
	.EVENT_BUF_ADDR           = 0x11A00,
	.RAW_PIPE0_ADDR           = 0x10000,
	.RAW_PIPE0_Q_ADDR         = 0,
	.RAW_PIPE1_ADDR           = 0x12000,
...
struct nvt_ts_trim_id_table_1732 {
	uint8_t id[NVT_ID_BYTE_MAX];
	uint8_t mask[NVT_ID_BYTE_MAX];
	const struct nvt_ts_mem_map_1732 *mmap;
	uint8_t carrier_system;
};

static const struct nvt_ts_trim_id_table_1732 trim_id_table[] = {
...
#if WAKEUP_GESTURE
const uint16_t gesture_key_array_1732[] = {
 ...
 *******************************************************/
int32_t CTP_I2C_READ_1732(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
 ...
 *******************************************************/
int32_t CTP_I2C_READ_DUMMY_1732(struct i2c_client *client, uint16_t address)
{
	uint8_t buf[8] = {0};
	int32_t ret = -1;

	ret = CTP_I2C_READ_1732(client, address, buf, 2);
	if (ret < 0)
		NVT_ERR("CTP_I2C_READ_DUMMY_1732 failed.(%d)\n", ret);
...
return:
	Executive outcomes. 1-- - succeed. -5-- - I / O error
 *******************************************************/
int32_t CTP_I2C_WRITE_1732(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
	struct i2c_msg msg;
	int32_t ret = -1;
	int32_t retries = 0;
...
return:
	n.a.
*******************************************************/
int nvt_sw_reset_idle_1732(void)
{
	int ret = 0;
	uint8_t buf[4]={0};
	...
	buf[0] = 0x00;
	buf[1] = 0xA5;
	CTP_I2C_WRITE_1732(ts_1732->client, I2C_HW_Address, buf, 2);
...
return:
	n.a.
 *******************************************************/
void nvt_bootloader_reset_1732(void)
{
	uint8_t buf[8] = {0};
 ...
 *******************************************************/
int32_t nvt_clear_fw_status_1732(void)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
 ...
 *******************************************************/
int32_t nvt_check_fw_status_1732(void)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
...
return:
	Executive outcomes. 0-- - succeed. -1-- - failed.
 *******************************************************/
int32_t nvt_check_fw_reset_state_1732(RST_COMPLETE_STATE_1732 check_reset_state)
{
	uint8_t buf[8] = {0};
	int32_t ret = 0;
	int32_t retry = 0;
 ...
 *******************************************************/
int32_t nvt_read_pid_1732(void)
{
	uint8_t buf[3] = {0};
	int32_t ret = 0;
	...
	buf[1] = 0x00;
	buf[2] = 0x00;
	CTP_I2C_READ_1732(ts_1732->client, I2C_FW_Address, buf, 3);

	ts_1732->nvt_pid = (buf[2] << 8) + buf[1];
	/*snprintf(ts_1732->nvt_pid, sizeof(ts_1732->nvt_pid), "%02X%02X", buf[2], buf[1]); */
...
return:
	Executive outcomes. 0-- - success. -1-- - fail.
 *******************************************************/
int32_t nvt_get_fw_info_1732(void)
{
	uint8_t buf[64] = {0};
...
info_retry:
	...
	CTP_I2C_READ_1732(ts_1732->client, I2C_FW_Address, buf, 17);
	ts_1732->fw_ver = buf[1];
	ts_1732->x_num = buf[3];
	ts_1732->y_num = buf[4];
	ts_1732->abs_x_max = (uint16_t)((buf[5] << 8) | buf[6]);
	ts_1732->abs_y_max = (uint16_t)((buf[7] << 8) | buf[8]);
	ts_1732->max_button_num = buf[11];
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		ts_1732->x_num = 18;
		ts_1732->y_num = 32;
		ts_1732->abs_x_max = 720;
		ts_1732->abs_y_max = 1520;
		ts_1732->max_button_num = 0;
		...
		if (retry_count < 3) {
			...
			NVT_ERR("Set default fw_ver=%d, x_num=%d, y_num=%d, \
					...
					ts_1732->fw_ver, ts_1732->x_num, ts_1732->y_num,
					ts_1732->abs_x_max, ts_1732->abs_y_max, ts_1732->max_button_num);
			ret = -1;
		}
	} else {
		ret = 0;
...
/*	int32_t retries = 0;*/
	...
	if (i2c_wr == 0) {	//I2C write
		ret = CTP_I2C_WRITE_1732(ts_1732->client, (str[0] & 0x7F), &str[2], str[1]);
		if (ret < 0) {
			NVT_ERR("error, ret = %d\n", ret);
			return -EIO;
		}
		...
		ret = CTP_I2C_READ_1732(ts_1732->client, (str[0] & 0x7F), &str[2], str[1]);
		if (ret < 0) {
			NVT_ERR("error, ret=%d\n", ret);
			return -EIO;
		}
...
{
	struct nvt_flash_data_1732 *dev;

	dev = kmalloc(sizeof(struct nvt_flash_data_1732), GFP_KERNEL);
	if (dev == NULL) {
...
{
	struct nvt_flash_data_1732 *dev = file->private_data;
...
return:
	n.a.
 *******************************************************/
void nvt_ts_wakeup_gesture_report_1732(uint8_t gesture_id, uint8_t point_data[64 + 2])
{
	int i = 0;
	...
	if ((gesture_id != GESTURE_DOUBLE_CLICK && gesture_id >= 12 && gesture_id <= 26 && gesture_id != 22 && gesture_id != 23 && gesture_id != 24) || (gesture_id == 30) || (gesture_id == 31)) {
		gsWakeUpTrace_1732.id = point_data[3];
		gsWakeUpTrace_1732.OClockwise = point_data[43];
		for (i = 0; i < 9; i++) {
			gsWakeUpTrace_1732.u16aX[i] = (point_data[4 * i + 5] << 8) + point_data[4 * i + 4];
			gsWakeUpTrace_1732.u16aY[i] = (point_data[4 * i + 7] << 8) + point_data[4 * i + 6];
		}

		VTI("input_id=[%02d],OClockwise[%02X]", gsWakeUpTrace_1732.id, gsWakeUpTrace_1732.OClockwise);
		for (i = 0; i < 9; i++) {
			VTI("%d(%02d,%02d)", i, gsWakeUpTrace_1732.u16aX[i], gsWakeUpTrace_1732.u16aY[i]);
		}
	}
}
#endif
...
static void nvt_parse_dt(struct device *dev)
{
	...
	ts_1732->irq_gpio = of_get_named_gpio_flags(np, "novatek,irq-gpio", 0, &ts_1732->irq_flags);
...
static void nvt_parse_dt(struct device *dev)
{
	ts_1732->irq_gpio = NVTTOUCH_INT_PIN;
}
#endif
 ...
 *******************************************************/
static int nvt_gpio_config(struct nvt_ts_data_1732 *ts_1732)
{
	int32_t ret = 0;

	/* request INT - pin (Input) */
	if (gpio_is_valid(ts_1732->irq_gpio)) {
		ret = gpio_request_one(ts_1732->irq_gpio, GPIOF_IN, "NVT-int");
		if (ret) {
...
#if NVT_TOUCH_ESD_PROTECT
void nvt_esd_check_enable_1732(uint8_t enable)
{
	if (enable) {
...
static void nvt_ts_work_func(void *work)
{
	...
	ret = CTP_I2C_READ_1732(ts_1732->client, I2C_FW_Address, point_data, POINT_DATA_LEN + 1);
	if (ret < 0) {
		NVT_ERR("CTP_I2C_READ_1732 failed.(%d)\n", ret);
		goto XFER_ERROR;
	}
/*
	//-- - dump I2C buf -- -
 ...
 */
	...
	if (nvt_fw_recovery(point_data)) {
		nvt_esd_check_enable_1732(true);
		goto XFER_ERROR;
	}
#endif
	...
	if (bTouchIsAwake_1732 == 0) {
		...
		nvt_ts_wakeup_gesture_report_1732(input_id, point_data);
		/*enable_irq(ts_1732->client->irq);*/
		enable_or_disable_irq(1);		
		mutex_unlock(&ts_1732->lock);
		return;
	...
	/* get the point num */
	finger_cnt = 0;
	for (i = 0; i < ts_1732->max_touch_num; i++) {
		position = 1 + 6 * i;
		input_id = (uint8_t)(point_data[position + 0] >> 3);
		if ((input_id > ts_1732->max_touch_num) || (input_id <= 0)) {
			continue;
		}
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	/*finger down (enter & moving) */
			...
			if ((input_x < 0) || (input_y < 0))
				continue;
			if ((input_x > ts_1732->abs_x_max) || (input_y > ts_1732->abs_y_max))
				continue;
			finger_cnt++;
		}
	}
...
{
	spin_lock(&ts_1732->irq_spinlock);
	if (ts_1732->irq_enable) {
	disable_irq_nosync(ts_1732->client->irq);
		VTD("disable_irq_nosync");
		ts_1732->irq_enable = 0;
	}
	spin_unlock(&ts_1732->irq_spinlock);
...
return:
	n.a.
 *******************************************************/
void nvt_stop_crc_reboot_1732(void)
{
	uint8_t buf[8] = {0};
	int32_t retry = 0;
	buf[0] = 0xFF;
	buf[1] = 0x01;
	buf[2] = 0xF6;
	CTP_I2C_WRITE_1732(ts_1732->client, I2C_BLDR_Address, buf, 3);
	buf[0] = 0x4E;
	CTP_I2C_READ_1732(ts_1732->client, I2C_BLDR_Address, buf, 4);
	if (((buf[1] == 0xFC) && (buf[2] == 0xFC) && (buf[3] == 0xFC)) ||
		...
		for (retry = 5; retry > 0; retry--) {
			...
			buf[0] = 0x35;
			buf[1] = 0xA5;
			CTP_I2C_WRITE_1732(ts_1732->client, I2C_BLDR_Address, buf, 2);
			buf[0] = 0xFF;
			buf[1] = 0x03;
			buf[2] = 0xF1;
			CTP_I2C_WRITE_1732(ts_1732->client, I2C_BLDR_Address, buf, 3);
			buf[0] = 0x35;
			buf[1] = 0x00;
			CTP_I2C_READ_1732(ts_1732->client, I2C_BLDR_Address, buf, 2);
			if (buf[1] == 0xA5)
				break;
		}
		if (retry == 0)
...
static int8_t nvt_ts_check_chip_ver_trim(void)
{
	...
	nvt_bootloader_reset_1732(); /*NOT in retry loop */
	//---Check for 5 times---
	for (retry = 3; retry > 0; retry--) {
		/*nvt_bootloader_reset_1732();*/
		ret = nvt_sw_reset_idle_1732();
		if (ret < 0) {
			break;
		}
		...
		buf[5] = 0x00;
		buf[6] = 0x00;
		ret = CTP_I2C_READ_1732(ts_1732->client, I2C_BLDR_Address, buf, 7);
		if (ret < 0) {
			break;
		}
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table_1732)); list++) {
			found_nvt_chip = 0;
			...
			if (found_nvt_chip) {
				...
				ts_1732->mmap = trim_id_table[list].mmap;
				ts_1732->carrier_system = trim_id_table[list].carrier_system;
				ret = 0;
				goto out;
			} else {
				ts_1732->mmap = NULL;
...
static int nt_get_gesture_point(u16 *data)
{
	...
	VTI("enter");
	for (i = 0; i < 9; i++) {
		data[i * 2]  = gsWakeUpTrace_1732.u16aX[i];
		data[i * 2 + 1] = gsWakeUpTrace_1732.u16aY[i];
	}
...
extern int nt_get_ic_fw_version_1732(void);
static int nt_get_fw_version(int which)
{
	...
	if (FW_VERSION == which) {
		ver = nt_get_ic_fw_version_1732();
	}

	return ver;
}

extern int hand_update_firmware_1732(const struct firmware *firmware);
static int nt_fw_update(const struct firmware *firmware)
{
	...
	int ret = 0;
	ret = hand_update_firmware_1732(firmware);
	return ret;
}
...
extern int8_t nvt_customizeCmd_WaitSet_1732(uint8_t u8WaitAddr, uint8_t u8WaitStatus, uint8_t u8Cmd);
...
static void enable_or_disable_irq_wake(int enable)
{
	...
	if (enable) {
		enable_irq_wake(ts_1732->client->irq);
		VTD("enable_irq_wake");
	} else {
		disable_irq_wake(ts_1732->client->irq);
...
static void enable_or_disable_irq(int enable)
{
	...
	spin_lock_irqsave(&ts_1732->irq_spinlock, irqflags);

	if (ts_1732->irq_enable == enable) {
		spin_unlock_irqrestore(&ts_1732->irq_spinlock, irqflags);
		return;
	}
	if (enable) {
		enable_irq(ts_1732->client->irq);
...
static int nt_mode_change(int which)
{
	...
	if (which == TOUCHSCREEN_NORMAL) {
		...
		nvt_ts_resume(&ts_1732->client->dev);
	...
	if (which == TOUCHSCREEN_GESTURE) {
		...
		if (TOUCHSCREEN_SLEEP == last_ts_state) {
			...
			mdelay(40);
			VTI("JUST FOR DEBUG");
			nvt_customizeCmd_WaitSet_1732(EVENT_MAP_RESET_COMPLETE, RESET_STATE_INIT, 0x13);
		}
...
static int bbk_get_board_version(void)
{
	...
	if (*(version + 2) == '0') {
		VTI("This borad is PD1732D");
		return 0;
	} else {
		VTI("This borad is PD1732");
		return 1;
	}
	VTI("This borad is unknown, just set it PD1732");
	return 1;
...
extern int vivo_nt_set_charger_bit_1732(int state);
extern int vivo_nt_idle_ctl_1732(int state);
extern int vivo_nt_set_edge_restain_switch_1732(int on);
extern int vivo_nt_get_rawordiff_data_1732(int which, int *data);
extern int vivo_nt_readUdd_1732(unsigned char *udd);
extern int vivo_nt_writeUdd_1732(unsigned char *udd);
extern int bbk_nt_sensor_test_1732(char *buf, void *pdata, int tmp);
#include "fw_tm_1732.h"
#include "fw_xl_1732.h"
#include "fw_boe_1732.h"
#include "fw_tm_1732d.h"
#include "fw_inx_1732.h"
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (0x10 == nt_get_lcm_id()) {
	vts_data->tsDimensionX = 720;		//pdata->x_max;	
	vts_data->tsDimensionY = 1440;		//pdata->y_max;	
	} else {	//TM6.22
		vts_data->tsDimensionX = 720;		//pdata->x_max; 
	...
	vts_data->lcmNoiseTestKey = "com.nttouchscreen.deltadiff:MainActivity:null:null:null";
	...
	vts_data->getRawOrDiffData = vivo_nt_get_rawordiff_data_1732;
	...
	vivoTsFwAdd(VTS_FW_TYPE_FW,     vts_fw_tm_1732,     sizeof(vts_fw_tm_1732),     "PD1732",  VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0x11);
	vivoTsFwAdd(VTS_FW_TYPE_FW,     vts_fw_tm_1732,     sizeof(vts_fw_tm_1732),     "PD1732",  VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0x12);
	vivoTsFwAdd(VTS_FW_TYPE_FW,     vts_fw_xl_1732,     sizeof(vts_fw_xl_1732),     "PD1732",  VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0x00);
	vivoTsFwAdd(VTS_FW_TYPE_FW,     vts_fw_boe_1732,     sizeof(vts_fw_boe_1732),   "PD1732",  VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0x30);
	if (bbk_get_board_version() == 1) {
		vivoTsFwAdd(VTS_FW_TYPE_FW,     vts_fw_tm_1732,     sizeof(vts_fw_tm_1732),     "PD1732",  VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0x13);
	} else {
		vivoTsFwAdd(VTS_FW_TYPE_FW,     vts_fw_tm_1732d,     sizeof(vts_fw_tm_1732d),     "PD1732",  VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0x13);
	}
	vivoTsFwAdd(VTS_FW_TYPE_FW,     vts_fw_inx_1732,     sizeof(vts_fw_inx_1732),   "PD1732",  VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0x50);
	
	ts_1732 = kmalloc(sizeof(struct nvt_ts_data_1732), GFP_KERNEL);
	if (ts_1732 == NULL) {
	...
	ts_1732->client = client;
	i2c_set_clientdata(client, ts_1732);

	/*-- - parse dts-- - */
	nvt_parse_dt(&client->dev);

	/*-- - request and config GPIOs-- - */
	ret = nvt_gpio_config(ts_1732);
	if (ret) {
	...
	mutex_init(&ts_1732->lock);
	spin_lock_init(&ts_1732->irq_spinlock);
	ts_1732->irq_enable = 2;
	...
	ts_1732->max_touch_num = TOUCH_MAX_FINGER_NUM;

#if TOUCH_KEY_NUM > 0
	ts_1732->max_button_num = TOUCH_KEY_NUM;
#endif

	ts_1732->int_trigger_type = INT_TRIGGER_TYPE;

#if WAKEUP_GESTURE
	/*wake_lock_init(&gestrue_wakelock, WAKE_LOCK_SUSPEND, "poll-wake-lock"); */
#endif

	snprintf(ts_1732->phys, 1023, "input/ts_1732");

	/*-- - set int - pin & request irq-- - */
	client->irq = gpio_to_irq(ts_1732->irq_gpio);
	if (client->irq) {
		VTI("int_trigger_type=%d", ts_1732->int_trigger_type);
#if 0
#if WAKEUP_GESTURE
		ret = request_irq(client->irq, nvt_ts_irq_handler, ts_1732->int_trigger_type | IRQF_NO_SUSPEND, client->name, ts_1732);
#else
		ret = request_irq(client->irq, nvt_ts_irq_handler, ts_1732->int_trigger_type, client->name, ts_1732);
#endif
#endif
	ret = vivoTsInterruptRegister(client->irq, NULL, nvt_ts_irq_handler, ts_1732->int_trigger_type/* | IRQF_NO_SUSPEND | IRQF_ONESHOT */, ts_1732);
	...
	INIT_DELAYED_WORK(&ts_1732->nvt_fwu_work, Boot_Update_Firmware_1732_1732);
	/* please make sure boot update start after display reset(RESX) sequence */
	/*queue_delayed_work(nvt_fwu_wq, &ts_1732->nvt_fwu_work, msecs_to_jiffies(14000)); */
	Boot_Update_Firmware_1732_1732(NULL);
#endif
...
err_init_NVT_ts:
#endif
	free_irq(client->irq, ts_1732);
#if BOOT_UPDATE_FIRMWARE
err_create_nvt_fwu_wq_failed:
#endif
err_int_request_failed:
	mutex_destroy(&ts_1732->lock);
err_chipvertrim_failed:
err_check_functionality_failed:
	gpio_free(ts_1732->irq_gpio);
...
static int32_t nvt_ts_suspend(int status)
{
	...
	/*	enable_irq_wake(ts_1732->client->irq);// to do */

		VTI("Enabled touch wakeup gesture");

	} else if (status == TOUCHSCREEN_SLEEP) {
		/*disable_irq_wake(ts_1732->client->irq);//to do */
	...
	buf[2] = 0xFF;
	buf[3] = 0xFF;
	CTP_I2C_WRITE_1732(ts_1732->client, I2C_FW_Address, buf, 4);
#else
	CTP_I2C_WRITE_1732(ts_1732->client, I2C_FW_Address, buf, 2);
#endif
...
#else /* WAKEUP_GESTURE */
	disable_irq(ts_1732->client->irq);
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	nvt_bootloader_reset_1732();
	nvt_check_fw_reset_state_1732(RESET_STATE_REK);
...
static struct of_device_id nvt_match_table[] = {
	{ .compatible = "novatek,NVT-ts-1732",},
	{ },
};
#endif
/*
...
/*0x11 0x12 is TM, 0x00 is xl, 0x13 is TM_b, 0x30 is BOE,0x50 is TXD*/
	if ((0x11 != nt_get_lcm_id()) && (0x12 != nt_get_lcm_id()) && (0x00 != nt_get_lcm_id()) && (0x13 != nt_get_lcm_id()) && (0x30 != nt_get_lcm_id()) && (0x50 != nt_get_lcm_id())) {	/*id not confirmed now */
	...
	if (strcmp(productName, "PD1732")) {
		VTI("The product name is not PD1732");
		return 0;
	}
/*

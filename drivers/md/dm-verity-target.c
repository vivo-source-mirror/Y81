...
/*lizonglin add start*/
static void verity_restart(struct dm_verity *v)
{

	if (v != NULL) {
		if (v->fec->dm_logging_off == 5) 
			return;
	}
			     ...
			     unsigned long long block)
{
	...
	v->hash_failed = 1;
/*lizonglin modify start*/
	/* if (v->corrupted_errs >= DM_VERITY_MAX_CORRUPTED_ERRS) */
		/* goto out; */
/*lizonglin modify end*/
	v->corrupted_errs++;
...
/*lizonglin modify start*/
	DMERR("[%s:%d] %s: %s block %llu is corrupted",
			__func__, __LINE__, v->data_dev->name, type_str, block);
/*lizonglin modify end*/
	if (v->corrupted_errs == DM_VERITY_MAX_CORRUPTED_ERRS)
...
static void verity_finish_io(struct dm_verity_io *io, int error)
{
	...
	bio->bi_error = 0;/*lizonglin modify */

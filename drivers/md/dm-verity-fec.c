...
/*lizonglin add start*/
static int fec_bv_memset(struct dm_verity *v, struct dm_verity_io *io, u8 *data,
		       size_t len)
{
	struct dm_verity_fec_io *fio = fec_io(io);
...
#define DM_VERITY_SKIP_TIME_MS		(3 * 1000) /* 3s */
...
/*lizonglin add start*/
	...
	if (!f->fec_threshold_blocks || f->is_fec_off) { /* skip FEC */
		if (iter) {
			fio->output_pos = 0;
			r = verity_for_bv_block(v, io, iter, fec_bv_memset);
		}
		goto done;
	}
/*lizonglin add end*/
	...
	fio->level--;
/*lizonglin add start*/
	if (!f->fec_cnt) /* clear fec_cnt if the FECs occur discontinuously */
		schedule_delayed_work(&f->fec_work, msecs_to_jiffies(DM_VERITY_SKIP_TIME_MS));

	f->fec_cnt++;
	f->fec_total_time_us += (unsigned long)ktime_to_us(ktime_sub(ktime_get(), start));
	if ((f->fec_cnt > f->fec_threshold_blocks)
			|| (f->fec_total_time_us > (f->fec_threshold_time_ms * 1000)))	{ /* skip FEC */
		if (!f->is_fec_off) {
			f->is_fec_off = true;
			cancel_delayed_work_sync(&f->fec_work);
			schedule_delayed_work(&f->fec_work, msecs_to_jiffies(DM_VERITY_SKIP_TIME_MS)); /* 3s */

			DMERR("[%s:%d] %s: FEC off, fec_cnt=%d, fec_total_time_us=%lu",
					__func__, __LINE__, v->data_dev->name,
					f->fec_cnt, f->fec_total_time_us);
		}
	}

	snprintf(verity_env, DM_VERITY_ENV_LENGTH, "%s=%d,%#llx",
		"DM_VERITY_ERR_BLOCK_NR", type, (unsigned long long)block);
	kobject_uevent_env(&disk_to_dev(dm_disk(md))->kobj, KOBJ_CHANGE, envp);

	DMERR("[%s:%d] %s: %sFEC block %llu (%u), lba %llu, fec_cnt %d, ret %d, time:%lu us ",
			__func__, __LINE__, v->data_dev->name,
			f->is_fec_off ? "Warn: Skip ":"",
			(unsigned long long)block, io->n_blocks,
			(unsigned long long)io->iter.bi_sector, f->fec_cnt, r,
			(unsigned long)ktime_to_us(ktime_sub(ktime_get(), start)));
/*lizonglin add end*/
	/*lizonglin add for allways reture 0 */
	r = 0;
	return r;
}
...
/*lizonglin add start*/
static ssize_t
show_fec_blks(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct dm_verity_fec *f = container_of(kobj, struct dm_verity_fec,
					       kobj_holder.kobj);

	return snprintf(buf, PAGE_SIZE, "%d\n", f->fec_threshold_blocks);
}

static ssize_t
set_fec_blks(struct kobject *kobj, struct kobj_attribute *attr,
		const char *buf, size_t count)
{
	...
	sscanf(buf, "%d", &value);
	f->fec_threshold_blocks = value;

	return count;
}
static struct kobj_attribute attr_fec_blks =
			__ATTR(fec_blks, S_IRUGO | S_IWUSR, show_fec_blks, set_fec_blks);

static ssize_t
show_fec_ms(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct dm_verity_fec *f = container_of(kobj, struct dm_verity_fec,
						   kobj_holder.kobj);

	return snprintf(buf, PAGE_SIZE, "%lu\n", f->fec_threshold_time_ms);
}

static ssize_t
set_fec_ms(struct kobject *kobj, struct kobj_attribute *attr,
		const char *buf, size_t count)
{
	struct dm_verity_fec *f = container_of(kobj, struct dm_verity_fec,
						   kobj_holder.kobj);
	...
	sscanf(buf, "%lu", &value);
	f->fec_threshold_time_ms = value;

	return count;
}
static struct kobj_attribute attr_fec_ms =
			__ATTR(fec_ms, S_IRUGO | S_IWUSR, show_fec_ms, set_fec_ms);
...
/*lizonglin add start*/
static ssize_t
show_dm_logging_status(struct kobject *kobj, struct kobj_attribute *attr,
					char *buf)
{
	struct dm_verity_fec *f = container_of(kobj, struct dm_verity_fec,
						   kobj_holder.kobj);
...
static ssize_t
set_dm_logging_status(struct kobject *kobj, struct kobj_attribute *attr,
		const char *buf, size_t count)
{
	struct dm_verity_fec *f = container_of(kobj, struct dm_verity_fec,
						   kobj_holder.kobj);
	...
	sscanf(buf, "%d", &value);
	f->dm_logging_off = value;
	DMERR("set the dm_logging_off status =%d",
				f->dm_logging_off);

	return count;
}

static struct kobj_attribute attr_dm_logging =
			__ATTR(dm_logging, S_IRUGO | S_IWUSR, show_dm_logging_status, set_dm_logging_status);
/*lizonglin add end*/
...
/*lizonglin add start*/
static void fec_delay_work(struct work_struct *work)
{
	struct dm_verity_fec *f =
		container_of(work, struct dm_verity_fec, fec_work.work);

	f->is_fec_off = false;
	f->fec_cnt = 0;
	f->fec_total_time_us = 0;

	DMERR("[%s:%d] FEC on", __func__, __LINE__);
}
/*lizonglin add end*/
...
/*lizonglin add start*/
	INIT_DELAYED_WORK(&f->fec_work, fec_delay_work);
	f->is_fec_off = false;
	f->fec_cnt = 0;
	f->fec_total_time_us = 0;
	f->fec_threshold_blocks = 5;
	f->fec_threshold_time_ms = 300;
	f->dm_logging_off = 0;
/*lizonglin add end*/	
	return 0;
}

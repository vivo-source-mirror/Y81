...
static struct CONN_CB_INFO *gConnCbInfo;
...
void connectivity_register_callback(struct CONN_CB_INFO *callback)
{
	gConnCbInfo = callback;
}
EXPORT_SYMBOL(connectivity_register_callback);

void connectivity_export_hif_dump(void)
{
	if (gConnCbInfo && gConnCbInfo->wlan_hif_dump)
		gConnCbInfo->wlan_hif_dump();
}
EXPORT_SYMBOL(connectivity_export_hif_dump);

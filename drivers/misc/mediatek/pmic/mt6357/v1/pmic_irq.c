...
{
	printk("[pwrkey_int_handler] Press pwrkey %d\n",
...
{
#if 0//def CONFIG_VIVO_CHARGING_NEW_ARCH
		int vbat = battery_get_vbat();
		int I = get_bat_charging_current_level();
		int sign = meter_get_i_sign();
	
		if (sign)
				I = I - (I * 2);
	
		printk("~~~ pwrkey (vbat=%d,I=%d)\n", vbat, I);
#endif
...
#ifdef CONFIG_VIVO_CHARGING_NEW_ARCH
void pmic_chrdct_int_switch(bool enable)
{
        if (enable) {
                //pmic_register_interrupt_callback(39, chrdet_int_handler);
                pmic_enable_interrupt(INT_CHRDET_EDGE, 1, "PMIC");
        } else {
                pmic_enable_interrupt(INT_CHRDET_EDGE, 0, "PMIC");
        }
}
EXPORT_SYMBOL(pmic_chrdct_int_switch);
#endif

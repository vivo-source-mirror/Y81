 ...
 *============================================================================
 ****************************************************************************/
#define PFX "S5K3L6XXL1_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6XXL1_SENSOR_ID,
	...
	.custom1 = {
		.pclk = 480000000,				/* record different mode's pclk */                        
		.linelength  = 6176,				/* record different mode's linelength */                
		.framelength = 3232,			/* record different mode's framelength */                 
		.startx = 0,					/* record different mode's startx of grabwindow */            
		.starty = 0,					/* record different mode's starty of grabwindow */            
		.grabwindow_width  = 4208,		/* record different mode's width of grabwindow */     
		.grabwindow_height = 3120,		/* record different mode's height of grabwindow */    
	...
	.custom2 = {
		.pclk = 480000000,				/* record different mode's pclk */                  
		.linelength  = 6176,				/* record different mode's linelength */          
		.framelength = 3886,			/* record different mode's framelength */           
		.startx = 0,					/* record different mode's startx of grabwindow */      
		.starty = 0,					/* record different mode's starty of grabwindow */      
		.grabwindow_width  = 4208,		/* record different mode's width of grabwindow */
		.grabwindow_height = 3120,		/* record different mode's height of grabwindow */
		/* following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenari*/
		.mipi_data_lp2hs_settle_dc = 85,                                              
		/* following for GetDefaultFramerateByScenario()	*/                          
		.mipi_pixel_rate = 358400000,                                                 
		.max_framerate = 200,                                                         	
...
extern int s5k3l6xxl1_vivo_otp_read(void);
static int vivo_otp_read_when_power_on = 0;
extern otp_error_code_t S5K3L6XXL1_OTP_ERROR_CODE;
MUINT32  sn_inf_main_s5k3l6xxl1[13];  /*0 flag   1-12 data*/
extern unsigned int is_atboot;
extern unsigned  int s5k3l6xxl1_flag;
/*vivo hope add end*/
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		for (i = 0; i < timeout; i++) {
			mdelay(10);
			framecnt = read_cmos_sensor_16_16(0x0005);
			if ( framecnt == 0xFF) {
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
				*sensor_id = return_sensor_id() + 1;
				...
				vivo_otp_read_when_power_on = s5k3l6xxl1_vivo_otp_read();
				pr_debug("add:end read eeprom - when_power_on = %d\n",vivo_otp_read_when_power_on);
				/*vivo hope add end*/
				if (s5k3l6xxl1_flag == 1){
					pr_debug("i2c write id: 0x%x, sensor id: 0x%x, s5k3l6xxl1_flag = %d\n", imgsensor.i2c_write_id,*sensor_id, s5k3l6xxl1_flag);
					return ERROR_NONE;
				}
			}
			pr_debug("Read sensor id fail, id: 0x%x or s5k3l6xxl1_flag = %d\n",
				imgsensor.i2c_write_id, s5k3l6xxl1_flag);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
	}
	if (*sensor_id != imgsensor_info.sensor_id ||s5k3l6xxl1_flag != 1) {
	...
	case SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME:/*lzl*/
        ...
        case SENSOR_FEATURE_GET_CUSTOM_INFO:
            switch (*feature_data) {
				...
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5K3L6XXL1_OTP_ERROR_CODE;//otp_state
		#if 0                    
		memcpy( feature_data+2, sn_inf_main_s5k3l6xxl1, sizeof(MUINT32)*13); 
				for (i = 0 ; i<13 ; i++ ){
				printk("sn_inf_main_s5k3l6xxl1[%d]= 0x%x\n", i, sn_inf_main_s5k3l6xxl1[i]);
				}
...
UINT32 S5K3L6XXL1_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{

...
#include "s5k3l6xxmipiraw_Sensor.h"

#define PFX "S5K3L6XX_EEPROM_OTP"
...
int s5k3l6xx_vivo_otp_read(void)
{
	...
	{
		/////ModuleInfo 
		if (VIVO_VENDOR_OFILM != vivo_otp_data[0x01]) ///xuyongfu need check 2nd vendor
		{
			S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
	...
	sn_inf_main_s5k3l6xx[0] = 0x00;
	#if 0
	for (i = 0; i < 12; i++) {
		sn_inf_main_s5k3l6xx[i +1] = (MUINT32)vivo_sub_otp_data[i + Sn_sub_addr];
		LOG_INF("sn_inf_main_s5k3l6xx[%d] = 0x%x, vivo_sub_otp_data[0x%x] = 0x%x\n", i+1, sn_inf_main_s5k3l6xx[i+1],	i +Sn_sub_addr, vivo_sub_otp_data[i + Sn_sub_addr]);
	}
	#endif
		...
		{
			if(i == 0)
				LOG_INF("[cfx++]read_S5K3L6XX_PDAF_data");
			if(i < 496)

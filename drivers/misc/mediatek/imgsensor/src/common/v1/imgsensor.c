...
int GPIO173;
int GPIO174;
int gpio171_value, gpio173_value, gpio174_value;
#endif
...
#if defined(CONFIG_MTK_CAM_PD1732)	
	pr_info(
	    "%s %d, gpio171_value =%d, gpio173_value =%d, gpio174_value =%d\n",
	    __func__,
	    atomic_read(&pgimgsensor->imgsensor_open_cnt), gpio171_value, gpio173_value, gpio174_value);
#else
	pr_info(
		"%s %d\n",
		__func__,
...
#if defined(CONFIG_MTK_CAM_PD1803)
static void imgsensor_DVDD_init(void)
{
	struct device_node *of_node = of_find_compatible_node(NULL, NULL, "mediatek,camera_hw");

	DVDD_GPIO98 = of_get_named_gpio(of_node, "gpio98", 0);
	pr_err("Get DVDD pin=%d.\n", DVDD_GPIO98);
}
#endif

#if defined(CONFIG_MTK_CAM_PD1732)
static void imgsensor_GPIO171(void)
{
	struct device_node *of_node = of_find_compatible_node(NULL, NULL, "mediatek,camera_hw");
	if(!of_node)
		pr_err("of_node for GPIO171 is Null.\n");
	GPIO171 = of_get_named_gpio(of_node, "gpio171", 0);	
...
static void imgsensor_GPIO174(void)
{
	struct device_node *of_node = of_find_compatible_node(NULL, NULL, "mediatek,camera_hw");
	if(!of_node)
		pr_err("of_node for GPIO174 is Null.\n");
	GPIO174 = of_get_named_gpio(of_node, "gpio174", 0);	
	pr_err("Get GPIO174 pin=%d.\n", GPIO174);
}
#endif

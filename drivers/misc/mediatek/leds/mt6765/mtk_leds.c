...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	if (level == old_brightness) {
		...
		if ((0 == level) && (silent_reboot == 1)) {
			...
			silent_count = 0;
			printk("[ddp_dsi_ioctl]silent_reboot  is clear %d\n", silent_reboot);
		}
	...
	case MT65XX_LED_MODE_CUST_LCM:
		...
		if (level == 0) {
				...
				if (silent_reboot == 0)
					disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BKG_OUT0);
				printk("cys lm3697 disable");
			} else {
				disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_BKG_OUT1);
				//level = level*8;
				if (level > panel_max_bl_level)
					level = panel_max_bl_level;
				printk("cys lm3697 ke level =%d,", level);

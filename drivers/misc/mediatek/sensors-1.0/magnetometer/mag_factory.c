				       ...
				       unsigned long arg)
{
	...
	case MSENSOR_IOCTL_SENSOR_ENABLE:
		...
		if (mag_factory.fops != NULL &&
			...
			pr_info("MSENSOR_ENABLE,en: %d, sample_period:%dms\n",
				flag, 5);
	...
	case MSENSOR_IOCTL_SELF_TEST:
		if (mag_factory.fops != NULL &&
			...
			pr_info("MSENSOR_IOCTL_SELF_TEST result %d\n", err);
		...
		return 0;
	case MSENSOR_IOCTL_ENABLE_MAG:
		if (copy_from_user(&magenable, ptr, sizeof(magenable)))
			return -EFAULT;
		if (mag_factory.fops != NULL &&
		    mag_factory.fops->enable_sensor != NULL) {
			pr_info("MSENSOR_ENABLE,en: %d, sample_period:%dms\n",
				magenable, 20);
			err = mag_factory.fops->enable_sensor(magenable, 20);
			if (err < 0) {
				pr_err(
					"MSENSOR_IOCTL_ENABLE_MAG fail!\n");
				return -EINVAL;
			}
		} else {
			pr_err("MSENSOR_IOCTL_ENABLE_MAG NULL\n");
			return -EINVAL;
		}
		return 0;
	default:

...
static int mag_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	if (event->flush_action == DATA_ACTION &&
		...
		pr_info("msensor self_test result %d\n", event->magnetic_t.status);
		/* 15 means pass, else is fail */
		if (event->magnetic_t.status == 1)
			atomic_set(&obj->selftest_status, 15);
		else
	...
	int64_t sample_periods_ms)
{
	...
	if (enabledisable == true) {
		...
		if (READ_ONCE(obj->android_enable) == true) {
			pr_info("maghub_factory_enable_sensor android is enable, do not enable again\n");
			return 0;
		} else {
			pr_info("maghub_factory_enable_sensor enable\n");
		}
	} else {
		WRITE_ONCE(obj->factory_enable, false);
		if (READ_ONCE(obj->android_enable) == true) {
			pr_info("maghub_factory_enable_sensor android is enable, could not disable gsensor\n");
			return 0;
		} else {
			pr_info("maghub_factory_enable_sensor disable\n");
		}
	}
...
static int maghub_factory_do_self_test(void)
{
	...
	pr_info("maghub_factory_do_self_test come in\n");
	ret = sensor_selftest_to_hub(ID_MAGNETIC);
	if (ret < 0) {
		pr_err("msensor_self_test send message to hub fail: %d!\n", ret);
		return -1;
	}

...
static int accelhub_ReadSensorData_vivo(char *buf, int bufsize)
{
	...
	sprintf(buf, "%d %d %d", acc[ACCELHUB_AXIS_X],
		acc[ACCELHUB_AXIS_Y], acc[ACCELHUB_AXIS_Z]);
	if (atomic_read(&obj->trace) & ACCELHUB_TRC_IOCTL)
		pr_debug("gsensor data: %s!\n", buf);
...
static ssize_t show_data_value(struct device_driver *ddri, char *buf)
{
	char strbuf[ACCELHUB_BUFSIZE];

	pr_debug("vivomotion get gsensor data\n");
	accelhub_ReadSensorData_vivo(strbuf, ACCELHUB_BUFSIZE);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);
}

static ssize_t store_sensor_cali(struct device_driver *ddri, const char *buf, size_t tCount)
{
	int32_t value[4] = {5, 5, 6, 6};
	int i, j = 0;
	char bufnew[31];
	int err = 0;
	int32_t acc_cali[3] = {0};

	pr_debug("store_sensor_cali: buf is %s\n", buf);
	for (i = 0; i <= 30; i++) {
		bufnew[i] = *(buf + i);
		if (*(buf+i) == '\0')
		break;
	...
	pr_debug("acc_cali: %d %d %d\n", acc_cali[0], acc_cali[1], acc_cali[2]);

	err = sensor_set_vivo_cmd_to_hub(ID_ACCELEROMETER, GSENSOR_SET_CALI, acc_cali);
	if (err < 0) {
...
static DRIVER_ATTR(data, 0444, show_data_value, NULL);
static DRIVER_ATTR(calidata, 0644, NULL, store_sensor_cali);
...
static struct driver_attribute *accelhub_attr_list[] = {
	...
	&driver_attr_data,       /*dump sensor data for vivo motion */
...
static int gsensor_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	if (event->flush_action == DATA_ACTION &&
		...
		pr_info("gsensor self_test result %d\n", event->accelerometer_t.status);
		atomic_set(&obj->selftest_status, event->accelerometer_t.status);
					 ...
					 int64_t sample_periods_ms)
{
	...
	if (enabledisable == true) {
		...
		if (READ_ONCE(obj->android_enable) == true) {
			pr_info("gsensor_factory_enable_sensor android is enable, do not enable again\n");
			return 0;
		} else {
			pr_info("gsensor_factory_enable_sensor enable\n");
		}
	} else {
		WRITE_ONCE(obj->factory_enable, false);
		if (READ_ONCE(obj->android_enable) == true) {
			pr_info("gsensor_factory_enable_sensor android is enable, could not disable gsensor\n");
			return 0;
		} else {
			pr_info("gsensor_factory_enable_sensor disable\n");
		}
	}
...
static int gsensor_factory_do_self_test(void)
{
	...
	pr_info("gsensor_factory_do_self_test come in\n");
	ret = sensor_selftest_to_hub(ID_ACCELEROMETER);
	if (ret < 0) {
		pr_err("gsensor_self_test send message to hub fail: %d!\n", ret);
		return -1;
	}

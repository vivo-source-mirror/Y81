				       ...
				       unsigned long arg)
{
	...
	case GSENSOR_IOCTL_INIT:
		...
		if (accel_factory.fops != NULL &&
			...
			pr_info("GSENSOR_IOCTL_INIT, enable: %d, s_p:%dms\n",
				flag, 5);
		} else {
	...
	case GSENSOR_IOCTL_SELF_TEST:
		...
		return 0;
	case GSENSOR_IOCTL_ENABLE_ACC:
		if (copy_from_user(&accenable, ptr, sizeof(accenable)))
			return -EFAULT;
		if (accel_factory.fops != NULL &&
		    accel_factory.fops->enable_sensor != NULL) {
			err = accel_factory.fops->enable_sensor(accenable, 5);
			if (err < 0) {
				pr_err("GSENSOR_IOCTL_ENABLE_ACC fail!\n");
				return -EINVAL;
			}
			pr_info("GSENSOR_IOCTL_ENABLE_ACC, enable: %d, s_p:%dms\n",
				accenable, 5);
		} else {
			pr_debug("GSENSOR_IOCTL_ENABLE_ACC NULL\n");
			return -EINVAL;
		}
		return 0;
		return 0;

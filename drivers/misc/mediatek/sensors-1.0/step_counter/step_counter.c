...
static int step_d_real_enable(int enable)
{
	...
	if (enable)
		cxt->mEnableSensor = cxt->mEnableSensor | 0x01;
	else
		cxt->mEnableSensor = cxt->mEnableSensor & (~0x01);
...
static int significant_real_enable(int enable)
{
	...
	if (enable)
		cxt->mEnableSensor = cxt->mEnableSensor | 0x02;
	else
		cxt->mEnableSensor = cxt->mEnableSensor & (~0x02);
...
static int step_c_enable_data(int enable)
{
	...
	if (enable)
		cxt->mEnableSensor = cxt->mEnableSensor | 0x04;
	else
		cxt->mEnableSensor = cxt->mEnableSensor & (~0x04);
...
static int floor_c_enable_data(int enable)
{
	...
	if (enable)
		cxt->mEnableSensor = cxt->mEnableSensor | 0x08;
	else
		cxt->mEnableSensor = cxt->mEnableSensor & (~0x08);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("step_c_store_active buf=%s\n", buf);
	mutex_lock(&step_c_context_obj->step_c_op_mutex);
	...
	/*vivo sensor team add for report first data direct*/
	if (true == cxt->step_c_ctl.is_report_input_direct && ID_STEP_COUNTER == handle && 1 == en) {
		pr_info("enablesensor: %d, last step counter: %d\n", cxt->mEnableSensor, last_step_counter);
		if (cxt->mEnableSensor & (~0x04))
			step_c_data_report(last_step_counter, 2);
		else
...
int step_c_data_report(uint32_t new_counter, int status)
{
	...
	if (cxt->is_first_data_after_enable) {
		cxt->is_first_data_after_enable = false;
		if (true == cxt->step_c_ctl.is_report_input_direct) {
			event.flush_action = DATA_ACTION;
			...
			if (err < 0)
				pr_err_ratelimited("00 event buffer full, so drop this data\n");
		} else {
			if (new_counter != last_step_counter) {
				...
				err = sensor_input_event(step_c_context_obj->mdev.minor, &event);
				if (err < 0)
					pr_err_ratelimited("11 event buffer full, so drop this data\n");
			} else {
				event.flush_action = DATA_ACTION;
				event.handle = ID_STEP_COUNTER;
				event.word[0] = 0;
				...
				err = sensor_input_event(step_c_context_obj->mdev.minor, &event);
				if (err < 0)
					pr_err_ratelimited("22 event buffer full, so drop this data\n");
			}
		}
	} else if (last_step_counter != new_counter) {
		event.flush_action = DATA_ACTION;
		event.handle = ID_STEP_COUNTER;
		event.word[0] = new_counter;
		last_step_counter = new_counter;

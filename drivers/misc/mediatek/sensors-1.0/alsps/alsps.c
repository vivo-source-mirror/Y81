...
atomic_t prox_state;
/*
*modified by vivo als team
*set PROX_STATE_FAR to 5
*/
enum ProxState {
	PROX_STATE_NEAR = 0,
	PROX_STATE_FAR = 5,
...
int als_data_report(int value, int status)
{
	...
	/*Modify be vivo sensor team begin*/
	if (true/*value != last_als_report_data*/) {
	/*Modify by vivo sensor team end REASON:we need light sensor to continue report the data*/
		event.flush_action = DATA_ACTION;
		event.word[0] = value;
		event.status = status;
		err = sensor_input_event(cxt->als_mdev.minor, &event);
...
int ps_data_report(int value, int status)
{
	...
	pr_notice("[ALS/PS]ps_data_report! %d(distance %d - raw_data %d)\n", value, distance, raw_data);
	event.flush_action = DATA_ACTION;
	/* modify by vivo sensor team begin */
	/* event.word[0] = value + 1; */
	event.word[0] = distance + 1;
	event.word[1] = raw_data;
	/* modify by vivo sensor team end */
...
/*vivo als team add for print log*/
...
/*vivo als team add for print log end*/
#if !defined(CONFIG_NANOHUB) || !defined(CONFIG_MTK_ALSPSHUB)
...
DEVICE_ATTR(debug, 0644, show_debug, store_debug);

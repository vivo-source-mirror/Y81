				...
				int64_t sample_periods_ms)
{
	...
	if (READ_ONCE(obj->als_android_enable) == true) {
		/* Android framework enable als, keep current state*/
		pr_err("alshub_factory_enable_sensor als_android_enable active so return\n");
		return 0;
	}
	/* Add by vivo sensor team end */
	if (enable_disable == true) {
			...
			int64_t sample_periods_ms)
{
	...
	if (READ_ONCE(obj->ps_android_enable) == true) {
		/* Android framework enable ps, keep current state*/
		pr_err("pshub_factory_enable_sensor ps_android_enable active so return\n");
		return 0;
	}
	/* Add by vivo sensor team end */
	if (enable_disable == true) {
...
/*add by vivo sensor team begin*/
static int pshub_factory_do_vivo_command(vivo_sensor_eng_cmd cmd, int32_t *data)
{
	int err = 0;
	uint8_t sensorType = ID_BASE;

	if (cmd == ALSPS_IOCTL_BOOST_ALS_REPORT) {
		err = sensor_batch_to_hub(ID_LIGHT, 0, *data, *(data + 1));
		pr_err("do_vivo_command (cmd)0x%x (val1)%d (val2)%d err(%d)\n", cmd, *data, *(data + 1), err);
		return 0;
	}

	switch (cmd) {
	case SENSOR_COMMAND_READ_PS_INT:
	case SENSOR_COMMAND_CHECK_PS_INT:
	case SENSOR_COMMAND_GET_PS_DATA_RANGE:
	case SENSOR_COMMAND_GET_PS_STATUS:
	case SENSOR_COMMAND_READ_PS_REG_DATA:
	case SENSOR_COMMAND_WRITE_PS_REG_DATA:
	case SENSOR_COMMAND_SET_PS_CALI_DATA:
	case SENSOR_COMMAND_SET_PS_ENG_CALI_DATA:
	case SENSOR_COMMAND_NOTIFY_PS_THRES_LEVEL:
	case SENSOR_COMMAND_GET_PS_PARA_INDEX:
		sensorType = ID_PROXIMITY;
	case SENSOR_COMMAND_GET_ALS_PARA_INDEX:
		sensorType = ID_LIGHT;
	default:
		break;
	}
	if (sensorType == ID_BASE)
		return -1;

	err = sensor_set_vivo_cmd_to_hub(sensorType, cmd, data);
	pr_err("pshub_factory_do_vivo_command (cmd)0x%x (val1)%d (val2)%d err(%d)\n", cmd, *data, *(data + 1), err);
	if (err < 0)
		return -1;
	return 0;
}
...
static struct alsps_factory_fops alspshub_factory_fops = {
	...
	/*add by vivo sensor team begin*/
	.do_vivo_commands = pshub_factory_do_vivo_command,
	/*add by vivo sensor team end*/
};

					 ...
					 unsigned long arg)
{
	...
	uint32_t cmd_args[VIVO_SENSOR_ENG_CMD_ARG_SIZE] = {0, 0};
	...
	case ALSPS_IOCTL_READ_PS_INT:
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_READ_PS_INT, cmd_args);
			if (err < 0) {
				pr_err("ALSPS_IOCTL_READ_PS_INT read data fail!\n");
				return -EINVAL;
			}
			data = cmd_args[0];
			pr_err("ALSPS_IOCTL_READ_PS_INT %d\n", data);
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
		} else {
	...
	case ALSPS_IOCTL_GET_PS_DATA_RANGE:
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_GET_PS_DATA_RANGE, cmd_args);
			if (err < 0) {
				pr_err("ALSPS_IOCTL_GET_PS_DATA_RANGE read data fail!\n");
				return -EINVAL;
			}
			data = cmd_args[0];
			pr_err("ALSPS_IOCTL_GET_PS_DATA_RANGE %d\n", data);
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
		} else {
	...
	case ALSPS_IOCTL_GET_PS_STATUS:
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_GET_PS_STATUS, cmd_args);
			if (err < 0) {
				pr_err("ALSPS_IOCTL_GET_PS_STATUS read data fail!\n");
				return -EINVAL;
			}
			data = cmd_args[0];
			pr_err("ALSPS_IOCTL_GET_PS_STATUS %d\n", data);
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
		} else {
	...
	case ALSPS_IOCTL_GET_PS_REG_DATA:
		if (copy_from_user(&cmd_args, ptr, sizeof(cmd_args)))
			return -EFAULT;
		pr_err("ALSPS_IOCTL_GET_PS_REG_DATA reg 0x%x\n", cmd_args[0]);
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_READ_PS_REG_DATA, cmd_args);
			if (err < 0) {
				pr_err("ALSPS_IOCTL_GET_PS_REG_DATA read data fail!\n");
				return -EINVAL;
			}
			pr_err("ALSPS_IOCTL_GET_PS_REG_DATA reg: 0x%x val: 0x%x\n", cmd_args[0], cmd_args[1]);
	...
	case ALSPS_IOCTL_SET_PS_REG_DATA:
		if (copy_from_user(&cmd_args, ptr, sizeof(cmd_args)))
			return -EFAULT;
		pr_err("ALSPS_IOCTL_SET_PS_REG_DATA reg 0x%x val 0x%x\n", cmd_args[0], cmd_args[1]);
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_WRITE_PS_REG_DATA, cmd_args);
			if (err < 0) {
	...
	case ALSPS_IOCTL_SET_PS_ENG_CALI_DATA:
		if (copy_from_user(&cmd_args, ptr, sizeof(cmd_args)))
			return -EFAULT;
		pr_err("ALSPS_IOCTL_SET_PS_ENG_CALI_DATA %d\n", cmd_args[0]);
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_SET_PS_ENG_CALI_DATA, cmd_args);
			if (err < 0) {
				pr_err("ALSPS_IOCTL_SET_PS_ENG_CALI_DATA fail!\n");
				return -EINVAL;
			}
			pr_err("ALSPS_IOCTL_SET_PS_ENG_CALI_DATA args:(%d ,%d)\n", cmd_args[0], cmd_args[1]);
			if (copy_to_user(ptr, &cmd_args, sizeof(cmd_args)))
				return -EFAULT;
		} else {
	...
	case ALSPS_IOCTL_NOTIFY_PS_THRES_LEVEL:
		if (copy_from_user(&cmd_args, ptr, sizeof(cmd_args)))
			return -EFAULT;
		pr_err("ALSPS_IOCTL_NOTIFY_PS_THRES_LEVEL %d\n", cmd_args[0]);
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_NOTIFY_PS_THRES_LEVEL, cmd_args);
			if (err < 0) {
				pr_err("ALSPS_IOCTL_NOTIFY_PS_THRES_LEVEL fail!\n");
				return -EINVAL;
			}
			pr_err("ALSPS_IOCTL_NOTIFY_PS_THRES_LEVEL args:(%d ,%d)\n", cmd_args[0], cmd_args[1]);
			if (copy_to_user(ptr, &cmd_args, sizeof(cmd_args)))
				return -EFAULT;
		} else {
	...
	case ALSPS_IOCTL_BOOST_ALS_REPORT:
		if (copy_from_user(&cmd_args, ptr, sizeof(cmd_args)))
			return -EFAULT;
		pr_err("ALSPS_IOCTL_BOOST_ALS_REPORT %d\n", cmd_args[0]);
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(ALSPS_IOCTL_BOOST_ALS_REPORT, cmd_args);
			if (err < 0) {
	...
	case ALSPS_IOCTL_GET_PS_PARA_INDEX:
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_GET_PS_PARA_INDEX, cmd_args);
			if (err < 0) {
				pr_err("SENSOR_COMMAND_GET_PS_PARA_INDEX read data fail!\n");
				return -EINVAL;
			}
			data = cmd_args[0];
			pr_err("SENSOR_COMMAND_GET_PS_PARA_INDEX %d\n", data);
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
		} else {
			pr_err("SENSOR_COMMAND_GET_PS_PARA_INDEX NULL\n");
			return -EINVAL;
		}
		return 0;
	case ALSPS_IOCTL_GET_ALS_PARA_INDEX:
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_GET_ALS_PARA_INDEX, cmd_args);
			if (err < 0) {
				pr_err("SENSOR_COMMAND_GET_ALS_PARA_INDEX read data fail!\n");
				return -EINVAL;
			}
			data = cmd_args[0];
			pr_err("SENSOR_COMMAND_GET_ALS_PARA_INDEX %d\n", data);
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
		} else {
			pr_err("SENSOR_COMMAND_GET_ALS_PARA_INDEX NULL\n");
			return -EINVAL;
		}
		return 0;
	...
	case ALSPS_IOCTL_SET_CALI:
		if (copy_from_user(&data, ptr, sizeof(data)))
			return -EFAULT;
		/* Modify by vivo sensor team begin For Proximty Calibration SCP*/
		/*if (alsps_factory.fops != NULL && alsps_factory.fops->ps_set_cali != NULL) {*/
		/*	err = alsps_factory.fops->ps_set_cali(data);*/
		cmd_args[0] = data;
		pr_err("ALSPS_IOCTL_SET_CALI cali data %d\n", data);
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			err = alsps_factory.fops->do_vivo_commands(SENSOR_COMMAND_SET_PS_CALI_DATA, cmd_args);
		/* Modify by vivo sensor team end For Proximty Calibration SCP*/
			if (err < 0) {

...
static void SCP_sensorHub_init_sensor_state(void)
{
	...
	mSensorState[ID_ANGLE_JUDGE].sensorType = ID_ANGLE_JUDGE;
	mSensorState[ID_ANGLE_JUDGE].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[ID_ANGLE_JUDGE].timestamp_filter = false;

	mSensorState[ID_AMD].sensorType = ID_AMD;
	mSensorState[ID_AMD].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[ID_AMD].timestamp_filter = false;

	mSensorState[ID_RAISEUP_DETECT].sensorType = ID_RAISEUP_DETECT;
	mSensorState[ID_RAISEUP_DETECT].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[ID_RAISEUP_DETECT].timestamp_filter = false;

	mSensorState[ID_PUTDOWN_DETECT].sensorType = ID_PUTDOWN_DETECT;
	mSensorState[ID_PUTDOWN_DETECT].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[ID_PUTDOWN_DETECT].timestamp_filter = false;

	mSensorState[ID_ANGLE_DIRECTION].sensorType = ID_ANGLE_DIRECTION;
	mSensorState[ID_ANGLE_DIRECTION].rate = SENSOR_RATE_ONCHANGE;
	mSensorState[ID_ANGLE_DIRECTION].timestamp_filter = false;
}
...
int sensor_set_vivo_cmd_to_hub(uint8_t sensorType, vivo_sensor_eng_cmd cmd, int32_t *data)
{
	/*
	*  !!!!Notice: data size will be VIVO_SENSOR_ENG_CMD_ARG_SIZE(2)!!!!
	*/
	SCP_SENSOR_HUB_DATA req;
	int len = 0, err = 0;
	...
	req.set_cust_req.vivoCmds.data[0] = cmd;
	req.set_cust_req.vivoCmds.data[1] = *data;
	req.set_cust_req.vivoCmds.data[2] = *(data + 1);
	if (ID_ACCELEROMETER == sensorType) {
		req.set_cust_req.vivoCmds.data[3] = *(data + 2);
	}
	...
	if (err == 0) {
		...
		pr_err("sensor_set_vivo_cmd_to_hub:(CMD)0x%x (val)%d val2(%d)\n", 
			req.set_cust_rsp.vivoCmds.data[0], req.set_cust_rsp.vivoCmds.data[1],
			req.set_cust_rsp.vivoCmds.data[2]);
		*(data) = req.set_cust_rsp.vivoCmds.data[1];
		*(data + 1) = req.set_cust_rsp.vivoCmds.data[2];
	} else {

...
{
	struct WriteAddrStruct *pWriteAddr;
	unsigned long flagsWriteAddr;
	...
	if (pa < start) {
		...
		list_for_each_entry(pWriteAddr, &gCmdqContext.writeAddrList, list_node)
			if (pa - (unsigned long)pWriteAddr->pa <
				pWriteAddr->count << 2) {
				ret = true;
				break;
			}
...
/*begin XieJiYuan@20190911@security check optimization*/
#define VIVO_CMDQ_CHECK_USER_OPTIMIZATION

#if defined(VIVO_CMDQ_CHECK_USER_OPTIMIZATION)
static bool cmdq_core_need_check_user(void)
{
	#undef AID_SHELL
	...
	/* refer to sys_geteuid() */
	uid_val = from_kuid_munged(current_user_ns(), current_euid());
	return (uid_val >= AID_SHELL);

	#undef AID_SHELL
}
#endif
/*end XieJiYuan@20190911@security check optimization*/
...
/*begin XieJiYuan@20190911@security check optimization*/
#if defined(VIVO_CMDQ_CHECK_USER_OPTIMIZATION)
	if (unlikely(userSpaceRequest && cmdq_core_need_check_user() &&
		!cmdq_core_check_task_valid(pTask)))
		return -EFAULT;
#else

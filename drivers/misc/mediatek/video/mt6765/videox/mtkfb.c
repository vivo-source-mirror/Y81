...
static bool fb_enter_early_suspend = false;
bool get_fb_suspend_state(void)
{
        return fb_enter_early_suspend;
}
EXPORT_SYMBOL(get_fb_suspend_state);
#endif
...
static void mtkfb_shutdown(struct platform_device *pdev)
{
	...
	if (primary_display_is_sleepd()) {
		...
		DISPERR("LCM mtkfb has been power off\n");
		return;
	}
	primary_display_set_power_mode(FB_SUSPEND);

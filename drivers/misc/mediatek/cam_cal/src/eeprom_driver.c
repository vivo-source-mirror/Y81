...
extern int vivo_sub_otp_read_when_power_on_hi556; /* add from hi556_mipi_raw_Sensor.c*/
extern unsigned char vivo_sub_otp_data_hi556[];/* add from hi556_mipi_raw_otp.c*/

extern int vivo_sub_otp_read_when_power_on_ov5695; /* add from ov5695_mipi_raw_Sensor.c*/
extern unsigned char vivo_sub_otp_data_ov5695[];/* add from ov5695_mipi_raw_otp.c*/
#endif
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		#if defined(CONFIG_MTK_CAM_PD1732)
		/*add for hi556*/
		if (ptempbuf->sensorID == 0x0556 && vivo_sub_otp_read_when_power_on_hi556 == 1) {
			if (ptempbuf->u4Length == 4 && ptempbuf->u4Offset == 0x0000) {/*read mtk calibration id*/
				PK_DBG("read Hi556 sensor_otp data start copy,length 1!\n");
				memcpy(pu1Params, &calibration_versiongned[0], 4);
				PK_DBG("read Hi556 sensor_otp Memcpy end!\n");
				/*for (i = 0; i < 4; i++) {
				PK_DBG("read Hi556 pu1Params[%d]:%d\n", i, pu1Params[i]);
				}*/
			} else if (ptempbuf->u4Length == 13 && ptempbuf->u4Offset == 0x0008) { /*read hi556 data*/
				PK_DBG("read Hi556 sensor_otp data start copy,length 12!\n");
				memcpy(pu1Params, &vivo_sub_otp_data_hi556[8], 13); /*awb means moduleinfo data + lsc data*/
				PK_DBG("read Hi556 sensor_otp Memcpy end!\n");
				/*for (i = 0; i < 16; i++) {
				PK_DBG("read Hi556 pu1Params[%d]:%d\n", i, pu1Params[i]);
				}*/
			...
			/*return i4RetValue;*/
		} else if (ptempbuf->sensorID == 0x0556 && vivo_sub_otp_read_when_power_on_hi556 != 1) {
			PK_DBG("read Hi556 kernel otp read failed!,vivo_sub_otp_read_when_power_on_hi556 = %d\n", vivo_sub_otp_read_when_power_on_hi556);
			return 0;
		}
		/* add for hi556*/
		/*add for ov5695*/
		if (ptempbuf->sensorID == 0x5695 && vivo_sub_otp_read_when_power_on_ov5695 == 1) {
			if (ptempbuf->u4Length == 4 && ptempbuf->u4Offset == 0x0000) {/*read mtk calibration id*/
				PK_DBG("read OV5695 sensor_otp data start copy,length 1!\n");
				memcpy(pu1Params, &calibration_versiongned[0], 4);
				PK_DBG("read OV5695 sensor_otp Memcpy end!\n");
				/*for (i = 0; i < 4; i++) {
				PK_DBG("read Hi556 pu1Params[%d]:%d\n", i, pu1Params[i]);
				}*/
			} else if (ptempbuf->u4Length == 7 && ptempbuf->u4Offset == 0x0000) { /*read ov5695 data*/
				PK_DBG("read OV5695 sensor_otp data start copy,length 12!\n");
				memcpy(pu1Params, &vivo_sub_otp_data_ov5695[0], 7); /*awb means moduleinfo data + lsc data*/
				PK_DBG("read OV5695 sensor_otp Memcpy end!\n");
				/*for (i = 0; i < 16; i++) {
				PK_DBG("read Hi556 pu1Params[%d]:%d\n", i, pu1Params[i]);
				}*/
			...
			/*return i4RetValue;*/
		} else if (ptempbuf->sensorID == 0x5695 && vivo_sub_otp_read_when_power_on_ov5695 != 1) {
			PK_DBG("read OV5695 kernel otp read failed!,vivo_sub_otp_read_when_power_on_ov5695 = %d\n", vivo_sub_otp_read_when_power_on_ov5695);
			return 0;
		}
		/* add for ov5695*/
		#endif

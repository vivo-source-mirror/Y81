...
static void ISP_EnableClock(bool En)
{
	...
	unsigned int setReg;
#endif
	unsigned int _regCheck;
	unsigned int _regCheck_rel;
	if (En) {
		...
		if (G_u4EnableClockCount == 0) {
			...
			_regCheck = ISP_RD32(CLOCK_CELL_BASE);
			pr_info("ALPS04024394 CLOCK_CELL_BASE value on is 0x(%x)",_regCheck);
		}
		G_u4EnableClockCount++;
		spin_unlock(&(IspInfo.SpinLockClock));
	...
	ISP_WR32(ISP_CAMSYS_CONFIG_BASE + 0x120, 0xFFFFFF4F);
		...
		if (G_u4EnableClockCount == 0) {
			...
			ISP_WR32(CLOCK_CELL_BASE, _reg&(~(1<<6)));
			_regCheck_rel = ISP_RD32(CLOCK_CELL_BASE);
           	pr_info("ALPS04024394 CLOCK_CELL_BASE off value is 0x(%x)",_regCheck_rel);
		}
		spin_unlock(&(IspInfo.SpinLockClock));

...
#ifdef CONFIG_VIVO_HAVE_NO_USBOE
static DEFINE_MUTEX(dpdm_mutex);
static DEFINE_MUTEX(hiz_mutex);
static DEFINE_MUTEX(recover_mutex);
bool usb_enable_clock(bool enable);
void usb_phy_recover(void);
bool dpdm_require_hiz = false;
static bool mtk_musb_is_init_done(void) {
    if (mtk_musb != NULL) {
        if (mtk_musb->xceiv != NULL) {
            if (mtk_musb->xceiv->io_priv != NULL) {
                return true;
            } else {
                printk("mtk_musb->xceiv->io_priv is NULL\n");
            }
        } else {
            printk("mtk_musb->xceiv is NULL\n");
        }
    } else {
        printk("mtk_musb is NULL\n");
    }
    return false;
}
int bc1p2_sw_en_state(void)
{
    int ret = 0;
    if (mtk_musb_is_init_done()) {
		if ((USBPHY_READ32(0x18) & (0x1 << 23)))
			ret = 1;
    }
    return ret;
}
static void bc1p2_sw_en_ctrl(void)
{
    int val = 0;
    ...
    if (mtk_musb_is_init_done()) {
        mutex_lock(&dpdm_mutex);
        if(dpdm_require_hiz)
           USBPHY_SET32(0x18, (0x1 << 23));
        else
            USBPHY_CLR32(0x18, (0x1 << 23));
        val = bc1p2_sw_en_state();
        printk("BC11_SW_EN =%d,dpdm_require_hiz=%d\n",val, dpdm_require_hiz);
        mutex_unlock(&dpdm_mutex);
    }
    return;
}
void dpdm_hiz_init(void)
{
    int ret = 0;

    mutex_lock(&hiz_mutex);
    dpdm_require_hiz = true;
    mutex_unlock(&hiz_mutex);
    ...
    if (mtk_musb_is_init_done()) {
        ...
        ret = bc1p2_sw_en_state();
        printk("BC11_SW_EN =%d\n",ret);

        //turn off internal 48Mhz PLL.
        usb_prepare_enable_clock(false);
        printk("dpdm_hiz_init end\n");
    }
}
void dpdm_hiz_release(void)
{
    mutex_lock(&hiz_mutex);
    dpdm_require_hiz = false;
    mutex_unlock(&hiz_mutex);

...
otg_disable_alarm_func(struct alarm *alarm, ktime_t now)
{
	...
	disable_irq_count++;
	if (otg_gpio_pin_ctrl->otg_pull_in && otg_gpio_pin_ctrl->otg_pull1_in && otg_gpio_pin_ctrl->otg_iddig_gpio) {
		if (!gpio_get_value(otg_gpio_pin_ctrl->platform_id) && otg_gpio_pin_ctrl->otg_chip_compatible_mode) {
			if (wake_lock_active(&otg_gpio_pin_ctrl->switchlock))
				wake_unlock(&otg_gpio_pin_ctrl->switchlock);
...
static void host_mode_enable(struct musb *musb, bool enable)
{
	if (host_mode_enabled ^ enable) {
		...
		if (enable) {
			...
			printk("musb_core start alarm \n");
			printk("usbid_gpio：%d, platform_id: %d\n", otg_gpio_pin_ctrl->usbid_gpio, otg_gpio_pin_ctrl->platform_id);
			if (!gpio_get_value(otg_gpio_pin_ctrl->platform_id) && otg_gpio_pin_ctrl->otg_chip_compatible_mode) {
				otg_gpio_pin_ctrl->id_pin_state = true;
				otg_gpio_pin_ctrl->last_id_pin_state = true;
				otg_gpio_pin_ctrl->id_state_change = false;
				wake_lock(&otg_gpio_pin_ctrl->switchlock);

				pinctrl_select_state(otg_gpio_pin_ctrl->pinctrl, otg_gpio_pin_ctrl->otg_pull_high);
				
				schedule_delayed_work(&otg_gpio_pin_ctrl->usbid_polling_work, msecs_to_jiffies(300));
			} else {
			...
			if (!gpio_get_value(otg_gpio_pin_ctrl->platform_id) && otg_gpio_pin_ctrl->otg_chip_compatible_mode) {
				...
				cancel_delayed_work_sync(&otg_gpio_pin_ctrl->usbid_polling_work);
			} else {
				disable_irq(iddig_eint_num);
				disable_irq_count++;
				pinctrl_select_state(otg_gpio_pin_ctrl->pinctrl, otg_gpio_pin_ctrl->otg_pull_in);

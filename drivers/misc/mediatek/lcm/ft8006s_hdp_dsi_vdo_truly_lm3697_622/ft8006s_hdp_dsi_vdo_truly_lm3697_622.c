...
static struct LCM_setting_table lcm_suspend_setting_enter_deepstandy[] = {
	{0x28, 0, {} },
	{0x10, 0, {} },
	{REGFLAG_DELAY, 120, {} },
	{0x04, 1, {0x5A} },
...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	params->dsi.vertical_backporch = 116;
	params->dsi.vertical_frontporch = 240;
	params->dsi.vertical_active_line = FRAME_HEIGHT;

	params->dsi.horizontal_sync_active = 14; //14
	params->dsi.horizontal_backporch = 10;   //160
	params->dsi.horizontal_frontporch = 12;  //120
...
{
	printk("%s ft8006s -> enter power on begin\n", __func__);
	lcm_vddi_setting(1);
	MDELAY(5);
	lcm_enp_setting(1);
	lcm_enn_setting(1);
	MDELAY(5);
	printk("%s ft8006s -> enter power on end\n", __func__);
}

static void lcm_suspend_power(void)
{
	pr_err("[LCM]ft8006s- lcm_suspend power off begin\n");
	MDELAY(10);
	lcm_enn_setting(0);
	lcm_enp_setting(0);
	MDELAY(7);
	#ifdef CONFIG_MTK_LCM_RESET_TOGGLE
	pr_err("[LCM]ft8006s- reset keep high level\n");
	#else
	if (lcm_ldo_vision) {
	lcm_reset_setting(0);
	tp_reset_setting(0);
	...
	#endif
	#ifdef CONFIG_MTK_LCM_RESET_TOGGLE
	pr_err("[LCM]ft8006s- vddi keep high level\n");
	#else
	if (lcm_ldo_vision) {
	lcm_vddi_setting(0);
	} else {
		pr_err("[LCM]ft8006s- vddi not control\n");
	}
	#endif
	panel_reset_state = 0;// clear reset state
	pr_err("[LCM]ft8006s- lcm_suspend power off end\n");
}
/*
...
static void lcm_init(void)
{
	...
	pr_err("[LCM]ft8006s- init end\n");
}

static void lcm_suspend(void)
{
	if (is_atboot) {
		push_table(lcm_suspend_setting_enter_deepstandy,
		sizeof(lcm_suspend_setting_enter_deepstandy) / sizeof(struct LCM_setting_table),
		1);
		pr_err("[LCM]ft8006s- lcm_suspend enter deepstandy\n");
	} else
		push_table(lcm_suspend_setting,
			sizeof(lcm_suspend_setting) / sizeof(struct LCM_setting_table),
			1);
	panel_off_state = 1;
	pr_err("[LCM]ft8006s- lcm_suspend display off\n");
}
...
static void tp_reset_for_incell(unsigned int status)
{
	if (status)
		tp_reset_setting(1);
	else
...
static unsigned int lcm_compare_id(void)
{
	...
	pr_debug("[LCM]%s,ft8006s debug: ft8006s id = 0x%08x\n", __func__, id);

	if (id == LCM_ID_ft8006s)
		return 1;
	else
		return 0;
...
struct LCM_DRIVER ft8006s_hdp_dsi_vdo_truly_lm3697_622_lcm_drv = {
	.name = "ft8006S_hdp_dsi_vdo_truly_lm3697_622_drv",

...
/*added by rongqianfeng start*/
extern unsigned int bsp_test_mode;
/*added by rongqianfeng end*/
...
#ifdef CONFIG_SWAP
#ifdef CONFIG_64BIT
/* modify by zhangzhiwei */
/*#define ENABLE_AMR_RAMSIZE	(0x60000) */	/* > 1.5GB */
#define ENABLE_AMR_RAMSIZE	(0xC0000 * 10)	/* > 3GB * 10 = 30GB, means disable AMR */
#else
...
#ifdef CONFIG_RSC_CHECK_LMK
void rsc_lmk_stat(u8 type, struct task_struct *task);
#endif
	...
	for_each_process(tsk) {
		...
		if (unlikely(bsp_test_mode))
			if (!strncmp(tsk->comm, "om.vivo.bsptest", 16) ||
				...
				printk("lowmemorykiller: skip task %s(pid %d), let it go\n", tsk->comm, tsk->pid);
				continue;
			}
		/*added by rongqianfeng end*/

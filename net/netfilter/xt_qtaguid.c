...
#define CONFIG_DO_NOT_COUNT_TCP_TO_OTHERS /* add by yanglinjun for filter to other ip tcps */
				       ...
				       struct xt_action_param *par)
{
	...
	int bytes = skb->len;
	int proto;
/* add by yanglinjun for filter to other ip tcps start */
#ifdef CONFIG_DO_NOT_COUNT_TCP_TO_OTHERS
	__be32 ip = 0;
#endif
/* add by yanglinjun for filter to other ip tcps end */
...
/* add by yanglinjun for filter to other ip tcps start */
#ifdef CONFIG_DO_NOT_COUNT_TCP_TO_OTHERS
	if (par->hooknum == NF_INET_PRE_ROUTING && proto == IPPROTO_TCP && !strcmp(el_dev->name, "wlan0")) {
		/*Now we check if wlan0 has ip address*/
		if (!el_dev || !(el_dev->ip_ptr) ||
		!((struct in_device *)(el_dev->ip_ptr))->ifa_list ||
		!(&(((struct in_device *)(el_dev->ip_ptr))->ifa_list->ifa_local))) {
			printk("qtaguid: %s: ip is not available.\n", __func__);
			/*we donnt have ip yet , do not count tcp*/
			return;
		} else{
			/*wlan0 has IP*/
			memcpy(&ip, &(((struct in_device *)(el_dev->ip_ptr))->ifa_list->ifa_local), sizeof(ip));
			if (ip != ip_hdr(skb)->daddr) {
				printk("qtaguid: %s: tcp dest ip is not me!!\n", __func__);
				return;
			}
		}
	}
